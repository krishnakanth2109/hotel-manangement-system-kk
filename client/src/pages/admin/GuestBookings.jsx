...
export function GuestList({ showCheckout = false }) {
  ...
  const columns = [
    ...
    ...(showCheckout ? [{
      key: 'actions', label: 'Actions', render: r => r.status === 'active' ? (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        ...
        <Table columns={columns} data={filtered} emptyMsg="No guests found" />
      </div>

      <Modal isOpen={!!confirmGuest} onClose={() => setConfirmGuest(null)} title="Confirm Check Out" size="sm">
        <div className="space-y-4">
          <p className="text-slate-600">Are you sure you want to check out <strong>{confirmGuest?.name}</strong> from Room <strong>#{confirmGuest?.roomNumber}</strong>?</p>
          <div className="flex gap-3 justify-end">
            <Button variant="secondary" onClick={() => setConfirmGuest(null)}>Cancel</Button>
            <Button variant="danger" onClick={() => { checkOutGuest(confirmGuest.id); setConfirmGuest(null); }}>Confirm Check Out</Button>
          </div>
        </div>
      </Modal>
    </div>
...
export function BookingList() {
  ...
  const columns = [
    { key: 'id', label: 'Booking ID', render: r => <span className="font-mono text-sm font-bold text-indigo-600">{r.id}</span> },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        ...
        <Table columns={columns} data={filtered} emptyMsg="No bookings found" />
      </div>
    </div>
  );
}
