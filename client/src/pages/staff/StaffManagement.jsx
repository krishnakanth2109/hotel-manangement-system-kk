...
const norm = (arr) => arr.map(s => ({ ...s, id: s._id || s.id }));
...
export default function StaffManagement() {
  ...
  const rows = norm(staff).filter(s => {
  ...
  const columns = [
    {
      ...
      render: r => (
        <div className="flex items-center gap-3">
          <Avatar initials={r.avatar || r.name?.slice(0,2).toUpperCase()} size="sm" color="indigo" />
          <div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        ...
        <Table columns={columns} data={rows} emptyMsg="No staff members found" />
      </div>
